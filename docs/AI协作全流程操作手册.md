# AI 协作全流程操作手册（从立项到上线 + 反哺）

> 这份手册是 [项目可视化流程手册.md](./项目可视化流程手册.md) 的"操作细则版"。
> 可视化手册告诉你"走哪条路、每步目标是什么"；这份手册告诉你"每一步具体怎么和 AI 对话、哪些必须你深度参与、哪些可以直接让它做、文档什么时候生成怎么改"。
>
> **用法**：每个新项目启动时，打开第 0 章判断工作模式；进入某个阶段前，翻该阶段的章节照着走。

---

## 序：三条铁律（写在最前面）

1. **AI 没有记忆，文档是它的记忆**——任何规则变更必须先落文档再改代码。
2. **AI 默认动手，你要逼它先想**——能讨论清楚的就不要让它边想边写。
3. **AI 默认讨好，你要逼它给判断**——禁止"列三方案让你选"，必须给唯一推荐。

---

## 阶段顺序总览（必须按此顺序）

```
1. 立项规划（含功能清单 + 阶段计划）
2. 技术选型
3. 架构设计文档（代码怎么组织、目录怎么放）
4. Agent 宪法（基于前 3 步的真实情况，一点点攒出来）
5. 实施真源文档拆解（大阶段 → 子阶段 → 步骤）
6. 前端骨架搭建
7. 数据库设计
8. 后端骨架搭建（生成后端架构实施文档）
9. 后端联调
10. 架构验收（后端 + 前端，生成两份真源文档）
11. 业务模块开发（一个模块一个模块写接口，边写边接前端，做完一个模块就验收 + 提交 git）
12. 后端安全审查
13. 上线前总验收
14. 上线 + Skill 反哺
```

**为什么是这个顺序**：

| 环节 | 为什么排在后面 |
|------|---------------|
| 架构设计文档在选型后 | 技术栈没定，目录结构无从谈起 |
| 宪法在架构设计文档后 | 一份好用的宪法必须结合项目的真实情况——立项、功能清单、技术栈，这些前面定下来 |
| 实施真源文档拆解在宪法后 | 宪法是长期原则，真源文档是阶段实施细则，原则先于细则 |
| 前端骨架在数据库前 | 前端骨架只搭壳不接数据，不依赖数据库 |
| 数据库在后端骨架前 | 后端骨架要连数据库验证，数据库得先就位 |
| 后端联调在后端骨架后 | 骨架跑通后才能联调，联调验证前后端是否接得上 |
| 后端验收在联调后 | 验收看实际实现，联调跑通才有实际实现可看 |
| 业务模块开发在验收后 | 地基不稳盖不了楼，验收通过才加业务 |

**宪法的攒法**：宪法不是一次性写完的，而是这个项目定一条规矩就一点点加。先有基础条款（禁止事项 + 验收标准 + 框架优先），后续每踩一个坑、每定一个规则，就往宪法里加一条。

**实施真源文档的拆解原则**：大阶段 → 子阶段 → 步骤，不是越细越好。三大核心：明确执行目标、划定任务边界、制定验收标准。完成首个阶段后，再拆解后续阶段，不需要一次性太细。如果要修改需求，业务细节可以随手调整，但地基（底层架构）不能随手翻。判断不了就直接问 AI 讨论。

---

## 第 0 章：先选你的工作模式

### 0.1 四段式工作循环（每个阶段都走一遍）

```mermaid
flowchart LR
    A[1.对话挖掘期] --> B[2.沉淀落地期]
    B --> C[3.执行交付期]
    C --> D[4.验收固化期]
    D --> A
```

| 段 | 谁主导 | 做什么 | 输出 |
|---|---|---|---|
| 1. 对话挖掘期 | **你主导** | 通过追问、反驳、要证据，把模糊需求挖清楚 | 讨论纪要（暂存对话里） |
| 2. 沉淀落地期 | AI 起草 + 你确认 | 把讨论结果写成文档（立项/选型/计划/真源文档） | 落盘的 .md 文件 |
| 3. 执行交付期 | AI 主导 | 按文档写代码 / 跑验收 / 修问题 | 代码 + 提交 |
| 4. 验收固化期 | 你判断 + AI 给证据 | 走验收清单、更新真源文档、Git 提交、反哺 Skill | 验收报告 + 文档同步 |

**核心原则**：第 1、4 段你必须深度参与；第 2 段你只需确认；第 3 段可以放手让 AI 做。

### 0.2 "对话挖掘" vs "直接执行" 判断标准

| 信号 | 模式 |
|---|---|
| 涉及产品方向、用户场景、功能边界、技术选型、规则定义 | 🗣️ **必须对话挖掘** |
| 涉及安全、权限、数据归属、商业模式 | 🗣️ **必须对话挖掘** |
| 涉及多个方案权衡、需要 ROI 分析 | 🗣️ **必须对话挖掘** |
| 你自己也说不清要什么 | 🗣️ **必须对话挖掘**（让 AI 反问你） |
| 已经有明确文档/规则/计划，只是落地实现 | ⚡ **可直接执行** |
| 重复性工作（建文件、改命名、批量替换、跑命令） | ⚡ **可直接执行** |
| 单一明确的 bug 修复（根因已定位） | ⚡ **可直接执行** |
| 格式化文档、整理清单、生成测试数据 | ⚡ **可直接执行** |

**一句话心法**：**判断模糊的事必须先聊，规则清晰的事才让它做。**

### 0.3 文档生成策略总表（含前缀命名规范）

**所有项目生成文档统一放项目根目录 `docs/`，按生成顺序加字母前缀**（`a-` → `q-`）。文件名排序 = 流程顺序，一眼看出推进到哪、下一步该生成什么。

| 前缀 | 文档（`docs/` 下） | 生成时机 | 修改方式 |
|---|---|---|---|
| `a-` | `a-立项文档.md` | 立项阶段讨论完后**一次性生成** | 仅在大方向变更时改 |
| `b-` | `b-功能清单.md` | 立项阶段讨论完后**一次性生成** | 功能变更时改 |
| `c-` | `c-阶段计划.md` | 立项阶段讨论完后**一次性生成** | 大方向变更时改 |
| `d-` | `d-选型.md` | 选型讨论完后**一次性生成** | 技术栈变更时改 |
| `e-` | `e-架构设计文档.md` | 选型后**一次性生成**，搭骨架时边补细节 | 目录结构 / 接口规则变更时改 |
| — | `AGENT_CONSTITUTION.md`（项目根目录，不在 docs/） | 架构设计文档后**先写基础条款，一点点攒** | 每定一条规矩就加一条，长期原则变更时改 |
| `f-` | `f-实施真源文档-第N阶段.md`（每阶段 1 份） | 宪法后**先拆首个阶段，后续边做边拆** | 完成一个阶段后拆下一个，避免一次性太细 |
| `g-` | `g-前端架构实施文档.md` | 前端骨架阶段生成 | 前端规则变更时改 |
| `h-` | `h-数据库设计文档.md` + `db/schema.sql` | 数据库设计阶段**先出文件再建表** | 每次结构变更先改文件 |
| `i-` | `i-后端架构实施文档.md` | 后端骨架阶段生成 | 后端规则变更时改 |
| `j-` | `j-联调报告.md` | 后端联调阶段生成 | 联调问题记录 |
| `k-` | `k-后端架构实施真源文档.md` | 架构验收阶段**反推生成** | 每次规则变更**立即同步** |
| `l-` | `l-前端架构实施真源文档.md` | 架构验收阶段**反推生成** | 每次规则变更**立即同步** |
| `m-` | `m-架构验收报告.md` | 架构验收阶段**一次性生成** | 整改后追加 |
| `n-` | `n-模块清单.md` | 业务门期间逐轮维护 | 每完成一个模块更新 |
| `o-` | `o-安全审查报告.md` | 安全审查阶段生成 | 整改后追加 |
| `p-` | `p-上线前验收报告.md` | 上线前总验收阶段生成 | 整改后追加 |
| `q-` | `q-项目复盘报告.md` | 项目结束**一次性生成** | 持续迭代 |

> 规则：
> - **前缀即顺序**：`a-` 立项 → `b-` 功能清单 → `c-` 阶段计划 → `d-` 选型 → `e-` 架构设计 → `f-` 实施真源 → `g-` 前端实施 → `h-` 数据库 → `i-` 后端实施 → `j-` 联调 → `k/l-` 前后端真源 → `m-` 架构验收 → `n-` 模块清单 → `o-` 安全审查 → `p-` 上线前验收 → `q-` 复盘。
> - **宪法例外**：`AGENT_CONSTITUTION.md` 放项目根目录（IDE 强制读取位置），不参与 `docs/` 前缀序列。
> - **多实例文档**：`f-实施真源文档-第N阶段.md` 每阶段 1 份（`f-实施真源文档-第1阶段.md`）；`h-` 的 schema 放 `db/schema.sql` 不占前缀。
> - **功能子文档**：复杂功能从 `b-功能清单` 拆出去的子文档沿用 `b-` 前缀（如 `b-功能-订单流转.md`），清单里留索引、子文档标所属模块，保持双向关联。
> - 全栈一体项目可合并：`k-`/`l-` 合为一份 `k-全栈架构真源文档.md`，`m-` 也合并验收。

**核心原则**：
- **"先文档后代码"**：架构、数据库、计划类——必须文档先行
- **"边做边补"**：真源实施文档——验收阶段反推，规则变更时同步
- **"一次性生成"**：立项/选型/宪法——讨论清楚后一次写完
- **"持续反哺"**：Skill 库——每个项目至少改 1 条经验

### 0.4 项目形态适配（全栈 vs 前后端分离）

上面的 14 阶段是**前后端分离项目**的完整流程。不是所有项目都需要前后端分离——很多项目是全栈一体（Next.js / Nuxt 全栈、微信小程序云开发、单后端 API、纯前端工具站）。**先判断项目形态，再决定哪些阶段合并/简化/跳过**，不要硬套 14 阶段。

#### 项目形态判断

```
你的项目有独立的"前端工程"和"后端工程"吗？
├─ 有，两个工程分别部署 → 前后端分离 → 走完整 14 阶段
├─ 没有，前后端在一个工程里 → 全栈一体 → 按下表合并阶段
└─ 只有后端 API（无前端页面）→ 纯后端 → 跳过前端相关阶段
└─ 只有前端（无后端逻辑）→ 纯前端 → 跳过后端相关阶段
```

#### 全栈一体项目阶段适配表

| 原阶段 | 全栈一体怎么处理 | 理由 |
|---|---|---|
| 1 立项 → 5 实施真源文档 | **不变** | 与项目形态无关 |
| 6 前端骨架 + 8 后端骨架 | **合并为一个"全栈骨架"阶段** | 同一工程，目录/路由/接口/数据库一次搭好 |
| 7 数据库设计 | **不变**（必须先于骨架，骨架要连库验证） | — |
| 9 后端联调 | **简化为"骨架自测"** | 前后端同体，不存在跨域/格式对齐问题，跑通核心路径即可 |
| 10 架构验收 | **合并验收**（不分前端/后端真源文档，出一份"全栈架构真源文档"） | 一份文档对应一个工程 |
| 11 业务模块开发 | **不变**（仍是"一个模块一个模块写，边写边接"，只是接口和页面在同一工程里写） | — |
| 12 安全审查 + testing-strategy | **不变**（全栈项目同样要做安全审查和测试六步法） | 安全/测试与项目形态无关 |
| 13 上线验收 → 14 反哺 | **不变** | — |

#### 纯后端 / 纯前端项目的简化

- **纯后端 API 项目**（无前端页面）：跳过阶段 6（前端骨架）；阶段 9 联调改为"用 Postman/curl 自测接口"；阶段 10 只出后端真源文档。
- **纯前端工具站**（无后端逻辑、无数据库）：跳过阶段 7（数据库）、8（后端骨架）、12（安全审查可简化为 XSS/输入校验）；阶段 11 就是"一个页面一个页面写"。

#### 适配原则（不管什么形态都成立）

1. **立项 → 选型 → 架构设计 → 宪法** 这条主轴不能省——哪怕全栈项目也要先定目录结构、接口规则、错误处理规则。
2. **数据库设计**不能省（只要有数据存储）——全栈项目更要早定 schema。
3. **安全审查 + 测试六步法**不能省——与项目形态无关，全栈项目一样有越权、注入、假测试风险。
4. **"先文档后代码、边做边补、做完一块验收一块"**的心法在任何形态都成立。

> 一句话：**14 阶段是参考路径，不是铁律。但"先想后做、文档先行、分块验收"是铁律。** 根据项目形态合并/简化阶段，但不要跳过"想清楚"这一步。

### 0.5 工作台工具：生成器（companion）+ 智能体（agent）

除了"人工翻手册 + 复制 prompt"，知识库还有两个工程化工具，把规则自动注入/自动执行。**它们是辅助不是替代**——门禁卡、Prompt 模板库仍然是你判断和对话的主工具。

#### ① companion/ — 一键生成工作台文件（推荐，零成本）

`companion/` 是一个生成器：读 `modules/` 里的信息模块，按当前阶段渲染出 `AGENTS.md` + `PROJECT_STATUS.md` 到你的项目，IDE 里的 AI 每次会话自动读。

```bash
# 在目标项目根目录执行（stage 为当前阶段编号 1-15）
python <知识库路径>/Vibe_Coding/companion/generator/main.py --project . --stage 1
```

**stage 编号与门禁对应**（注意：这是生成器自身编号，和上面的 14 阶段模型偏移 +1）：

| 生成器 stage | 对应门禁 | 门禁硬指标何时注入 |
|---|---|---|
| 1-5 | 立项门 | stage 5 到达时注入立项门 7 项硬指标 |
| 6-11 | 架构门 | stage 11 到达时注入架构门 7 项硬指标 |
| 12-13 | 业务门 | stage 13 到达时注入业务门 8 项硬指标 |
| 14-15 | 上线门 | stage 15 到达时注入上线门 7 项硬指标 |

生成的 `AGENTS.md` 自带三个固定段：**状态判断（5 种状态随时用）** + **成本与心力（每次会话都注意，横切层）** + **门禁状态（临近门禁时自动带出硬指标清单）**。横切层意味着"上下文膨胀 / 心力消耗"的提醒不用你每阶段手动加，每次开会话 AI 都能看到。

#### ② agent/ — 工程化智能体（可选，想要"自动跑流程"时用）

`agent/` 把方法论固化成能"自动执行流程 + 自动收证据 + 自动判门禁"的智能体（`SKILL.md` 为主入口）。当前 **v0.4.0 已打通全量 4 门禁闭环**：

| 闭环 | 对应文件 | 覆盖阶段 |
|---|---|---|
| 立项门闭环 | `workflows/立项门闭环.md` + `gate-checks/立项门.md` | 1-4 |
| 架构门闭环 | `workflows/架构门闭环.md` + `gate-checks/架构门.md` | 5-10 |
| 业务门闭环 | `workflows/业务门闭环.md` + `gate-checks/业务门.md` | 11-12 |
| 上线门闭环 | `workflows/上线门闭环.md` + `gate-checks/上线门.md` | 13-14 |

**两种用法**：
- **WorkBuddy 里用**：把 `agent/` 复制到 `~/.workbuddy/skills/vibe-coding-engineer/`，对话里说"我想做个 XX 项目"自动触发；说"帮我验收，能不能过架构门"就走对应 `gate-checks/` 自动收集证据。
- **项目里用**：复制 `agent/` 到项目根目录，在项目的 `AGENTS.md` / `CLAUDE.md` 引用 `SKILL.md`；智能体按 `state-machine.md` 读 `PROJECT_STATUS.agent.md` 定位当前阶段，推进时逐项调用门禁验收。

**状态流转**：立项门通过 → `current_phase=5` → 架构门通过 → `current_phase=11` → 业务门通过 → `current_phase=13` → 上线门通过 → 项目收官（`current_phase=14`，state=4）。

> 演进原则：知识库（docs/skills/templates/workflows）是单一来源；`agent/` 只引用不复制，`companion/` 只生成不篡改。知识库更新，两者自动继承。

### 0.6 PROJECT_STATUS.md — 工作台日常操作

**PROJECT_STATUS.md 是你每天唯一要维护的工作台文件**（放在项目根目录）。日常开发只看它、只更新它，不翻知识库。它由 7 个区域组成：

| 区域 | 内容 | 什么时候动 |
|---|---|---|
| 项目信息 | 项目名 / 开始日期 / 当前门禁 | 初始化填一次 |
| 当前门禁 | 正在过哪个门（立项/架构/业务/上线） | 门禁推进时更新 |
| 验收清单 | 从门禁卡复制的勾选项 | 每完成一项勾一项 |
| 下一步 | 当前要做的 ≤3 件事 | 每天早上 / 每轮结束更新 |
| 工作日志 | 日期 + 做了什么 + 问题 | 每次工作结束追加一行 |
| 规则变更记录 | 改真源文档 / 宪法时记录 | 每次规则变更时追加 |
| 踩坑记录 | 踩坑 + 根因 + 是否反哺 | 每次踩坑时追加 |

#### 初始化（3 种方式，选 1 个）

```powershell
# 方式 A：从知识库复制模板（最常用）
Copy-Item "e:\FiveTierProjectSystem\01-Inbox\Vibe_Coding\templates\PROJECT_STATUS.md" ".\PROJECT_STATUS.md"

# 方式 B：用生成器生成（会同时生成 AGENTS.md + PROJECT_STATUS.md，stage=当前阶段 1-15）
python e:\FiveTierProjectSystem\01-Inbox\Vibe_Coding\companion\generator\main.py --project . --stage 1

# 方式 C：智能体版（agent 模式，机器可读字段 + 4 门禁验收清单，智能体自动维护）
Copy-Item "e:\FiveTierProjectSystem\01-Inbox\Vibe_Coding\agent\templates\PROJECT_STATUS.agent.md" ".\PROJECT_STATUS.md"
```

初始化后填「项目信息」，从门禁卡复制「验收清单」进来，`git add PROJECT_STATUS.md && git commit -m "chore: 初始化项目状态文件"`。

#### 每天早上（2 分钟）

1. 打开 `PROJECT_STATUS.md`
2. 看「当前门禁」→ 知道自己正在过哪个门
3. 看「下一步」→ 今天做什么
4. 看「工作日志」最后一行 → 上次做到哪

#### 每次工作结束（收尾 5 步）

1. 勾选「验收清单」里完成项
2. 「工作日志」追加一行：`| 日期 | 做了什么 | 问题 |`
3. 更新「下一步」（清掉做完的，写下轮要做的 ≤3 项）
4. 如有规则变更 → 「规则变更记录」追加一行
5. Git 提交：`git commit -m "feat: 完成 XX（含 XX 接口）"`

#### 门禁推进时

1. 验收通过 → 更新「当前门禁」为下一道门 + 填「进入时间」
2. 从下一张门禁卡复制新的「验收清单」进来（旧清单保留在下方或归档）
3. Git 提交：`git commit -m "chore: 通过 XX 门"`
4. 提示 AI：让它读更新后的 PROJECT_STATUS.md 继续

#### Git 提交节奏（配合状态文件）

- **每次状态文件更新 = 一次提交**：状态推进、验收勾选、日志追加都随功能提交一起走，不要攒
- **门禁通过必须单独提交**：`chore: 通过 XX 门`，作为可回滚的里程碑点
- **改坏了就回退**：`git log --oneline` 找最近稳定提交 → `git reset --hard <commit-id>`

> **核心心法**：PROJECT_STATUS.md 是**你的施工日志 + AI 的记忆锚点**。你不更新它，AI 第二天就忘了进度；你更新了它，AI 每次开会话都能从文件恢复上下文。改它永远比翻手册快。

### 0.7 新项目工作台初始化（复制什么、不复制什么）

**新项目第一天只复制 2 样东西进项目**，其余全部留在知识库、按需翻。下面把「复制 / 不复制 / 什么时候 / 作用」一次说清，含容易踩的盲区。

#### ✅ 必须复制到项目根目录（2 样）

| 复制什么 | 命令（在项目根目录执行） | 什么时候 | 作用 |
|---|---|---|---|
| `skills/` 整个目录 | `Copy-Item "e:\FiveTierProjectSystem\01-Inbox\Vibe_Coding\skills\*" ".\skills\" -Recurse` | 项目第一天 | **AI 的方法论规则**。不复制，AI 在项目里读不到任何 skill，验证/审查/测试全失效 |
| `PROJECT_STATUS.md` | `Copy-Item "e:\FiveTierProjectSystem\01-Inbox\Vibe_Coding\templates\PROJECT_STATUS.md" ".\PROJECT_STATUS.md"` | 项目第一天 | **你的施工日志 + AI 记忆锚点**，每天维护 |

> 想更省事：用生成器一步生成 `AGENTS.md` + `PROJECT_STATUS.md`（见 0.5 ①），但 `skills/` 仍要单独复制。

#### ❌ 不复制、留在知识库按需翻（5 样）

| 不复制什么 | 什么时候用 | 为什么留在知识库 |
|---|---|---|
| 4 张门禁卡（`templates/门禁卡-*.md`） | 过对应门禁时打开看 | **门禁卡会随知识库更新迭代**，复制到项目就冻结成旧版，永远不同步。项目里只有 PROJECT_STATUS 里的验收清单副本 |
| `docs/` 4 篇手册 | 遇到盲区时翻 | 查证资料，不需要进项目 |
| `workflows/全流程速查.md` | 快速回顾阶段时翻 | 同上 |
| `agent/`（可选） | 想要智能体自动跑流程时 | 按需复制，不是默认工作台 |
| `sources/`、`companion/` | 溯源 / 重新生成时用 | 知识库工具，不进项目 |

#### 🔄 复制模板但改名放 docs/（2 样，到阶段才做）

| 模板 | 什么时候 | 复制成 |
|---|---|---|
| `模块清单模板.md` | 进入业务门（阶段 11）时 | 项目 `docs/n-模块清单.md` |
| `项目复盘模板.md` | 上线后（阶段 14） | 项目 `docs/q-项目复盘报告.md` |

> 这两个不是第一天复制，是**到对应阶段才生成**，且要用带前缀的文件名（见 0.3 命名规范）。

#### ⚠️ 盲区（你没问但一定会踩）

1. **门禁卡不要复制进项目**——这是最常见的误解。项目里只需 PROJECT_STATUS 里的「验收清单」，门禁卡本体留在知识库，改版自动生效。
2. **`AGENT_CONSTITUTION.md` 是"创建"不是"复制"**——宪法必须结合项目真实情况定制（阶段 4 做），不是把某个模板文件拷过去。
3. **IDE 入口文件只写一行引用宪法**——`AGENTS.md`/`CLAUDE.md` 等入口文件里写"本项目遵守 AGENT_CONSTITUTION.md"，不要复制宪法内容，否则多处不同步。
4. **`db/`、`migrations/` 等目录按项目技术栈自建**——知识库不提供代码骨架模板，文档（`docs/` 前缀文件）才是知识库的产出物。
5. **`skills/` 复制一次后不用管**——它是规则库，项目推进中不修改；发现规则要更新时，改的是知识库本体，反哺流程见阶段 14。

#### 第一天初始化完整动作（复制后）

```powershell
git init
Copy-Item ... skills/  # 见上表
Copy-Item ... PROJECT_STATUS.md
# （可选）生成器生成 AGENTS.md + PROJECT_STATUS.md
git add .
git commit -m "chore: 项目初始化"
```

> 初始化完成标准：项目根目录有 `skills/` + `PROJECT_STATUS.md`（+ 可选 `AGENTS.md`），git 已 init 并首次提交。之后每天只碰 PROJECT_STATUS.md（见 0.6）。

### 0.8 接手做一半的项目（stage 定位 + 补建状态文件）

**场景**：拿到一个不是从 0 开始的项目——可能是自己做到一半、可能是别人做的。目标就两步：**定位当前 stage → 补建/更新 PROJECT_STATUS.md**，然后从当前门禁继续。

#### 第一步：stage 定位（4 个信号，按优先级用）

| 信号 | 怎么用 | 优先级 |
|---|---|---|
| ① PROJECT_STATUS.md | 有就直接读「当前门禁 + 当前阶段」→ 查 0.5 的 stage 映射表。注意 `current_phase`（14 阶段编号）与生成器 stage **差 1**（阶段 5 → stage 6） | 最准 |
| ② docs/ 前缀文档 | `ls docs/` 看已有哪些 a-q 前缀文档 → **取前缀最大的那个，它的下一档就是当前阶段**（只有 `h-数据库设计文档` 没有 `i-` → 卡在后端骨架 stage 9 附近） | 最实用 |
| ③ 代码现状 + git log | 骨架能跑 → stage 7-10；有业务模块在写 → 12；线上能访问 → 14-15；`git log --oneline` 看提交历史还原进度（"通过架构门"提交过 → 至少 stage 12） | 无文档时 |
| ④ 门禁反推 | 拿不准时用门禁卡**倒着验**：立项门 7 项能全过 → 已过；架构门 7 项能全过 → 已过……**卡在哪道门过不全，就停在哪道门内**，取该门禁 stage 区间 | 兜底 |

**stage → 门禁速查**：stage 1-5 = 立项门；6-11 = 架构门；12-13 = 业务门；14-15 = 上线门。

#### 文档残缺时怎么定位（半路项目常见，别慌）

**前提认知：①②③ 是"快线索"不是"必要条件"——它们只是加速器，缺了就往后退，最后总落在「代码能不能跑」上。最终裁决永远是④门禁反推，它只依赖运行时验证，与文档完备度无关。**

| 缺了什么 | 换成什么 | 怎么换 |
|---|---|---|
| 没有 a-q 前缀文档 | **看现有文档内容** | 打开 `docs/` 里任意文件，内容比文件名可靠：有"功能清单"结构 → 立项做过；有接口/目录规则 → 架构设计做过 |
| 连 `docs/` 目录都没有 | 直接跳到代码 | 用③代码现状：能启动吗 → 首页/接口长什么样 → 有没有业务模块代码 |
| 没有 git 历史（拷贝/解压来的） | 看代码 + 顺手 `git init` | 重建历史起点，按代码判断阶段 |
| 代码跑不起来（烂尾） | 先抢救再定位 | 有 git 就回退稳定版；没有就修到能启动，按"能跑的部分"估 |
| 全部都没有（纯黑盒） | 考古反推 | 让 AI 读代码反推三份真源文档 → 反推出来自然知道做到哪了 |

**为什么④永远可用**：门禁反推不看文件名、不看文档、不看 git，只问"项目实际能做什么"——前端能启动吗？数据库能建表查询吗？后端四条线跑通了吗？前后端能联调吗？这些是**运行时验证**，10 分钟就能定位出卡在架构门内还是已进业务门。

**定位错了怎么办**：成本极低，放心试。stage 只是给生成器的提示参数，只生成 AGENTS.md / PROJECT_STATUS.md，**不碰代码**；错了重跑一次即可。且 stage 不是定死的——推进中发现"生成器提示的下一步"和实际产出对不上，立即重跑校准。不确定时**取保守档（往前靠一档）**，靠门禁反推在推进中动态修正。

**文档残缺时的最小定位路径**：

```
1. 跑起来看：npm run dev / go run / 启动脚本 → 能跑吗？
2. 能跑 → 打开首页 / 打几个接口 → 有业务功能吗？
   ├─ 只有骨架（健康检查页）→ 架构门内（stage 7-10）
   ├─ 有业务模块在写 → 业务门（stage 12）
   └─ 能上线访问 → 上线门（stage 14-15）
3. 按判断跑生成器 --stage N，生成后手动改 PROJECT_STATUS 填真实进度
4. 边推进边校准：AI 提示的下一步和实际不符 → 重跑 stage
```

#### 第二步：补建 / 更新 PROJECT_STATUS.md

3 种方式和第一天一样，但**填的是真实进度，不是从空开始**：

| 方式 | 中途用要注意 | 推荐度 |
|---|---|---|
| 复制模板 | 拿到空模板，手动填当前门禁 / 已完成验收项 / 下一步 | ⭐ 最推荐 |
| 生成器 `--stage N` | 传对 stage，但"下一步"是标准话术，仍要手动校正成真实进度 | 能用 |
| agent 版复制 | frontmatter 的 `current_state / current_phase / current_gate` 三个机器字段**必须手动填成实际值**，否则 AI 会从 0 开始 | 能用 |

**标准动作**：

```powershell
1. 定位：先跑起来看代码现状（最快）→ 再看 ls docs/ 前缀 + git log（有则辅助）
2. 复制：Copy-Item "...\templates\PROJECT_STATUS.md" ".\PROJECT_STATUS.md"
3. 填：当前门禁 / 进入时间 / 下一步（≤3 项）
4. 勾：从当前门禁卡复制验收清单 → 已做完的项直接勾 ✅
5. 工作日志从今天开始记（历史可留空）
6. 提交：git add PROJECT_STATUS.md && git commit -m "chore: 补建项目状态文件"
```

#### ⚠️ 三个注意点（最容易翻车的）

1. **别从立项门清单开始勾**——你已经过了的门直接标记通过，只把当前门禁的清单完整勾出来，AI 才不会误解进度。
2. **烂尾项目先回退再定位**——代码跑不起来，先 `git log --oneline` 找稳定提交回退，否则按坏代码定位会错得离谱。
3. **黑盒项目先补文档再定位**——能跑但没文档，定位 stage 后顺手让 AI 反推三份真源文档，否则后续每阶段缺依据。

> 承接关系：定位用 0.5 的 stage 映射，补建后维护用 0.6，完整复制清单见 0.7。接手项目的完整抢救流程见 [项目可视化流程手册](../docs/项目可视化流程手册.md)。

---

## 第 1 章：阶段 1 — 项目立项

### 1.1 阶段目标
把脑子里的模糊想法变成可执行的立项文档，明确第一版做什么、不做什么。

### 1.2 ⏱️ 预计工作量
2-3 轮对话，1 次文档落地，1 次 Git 提交。

### 1.3 🗣️ 对话挖掘期（你必须主导）

**核心挖掘点**（每条都要聊清楚，不能跳）：
- 项目是什么类型？面向谁？解决什么问题？
- 核心功能有哪些？数据怎么流转？商业模式是什么？
- 用户场景是什么？核心操作流程是什么？
- 第一版 MVP 边界是什么？哪些功能后面再做？哪些暂时放弃？
- 长期版本往哪里走？
- 类似产品、竞品、开源方案？差异点和机会在哪？
- **盲区追问**：权限、数据结构、状态字段、后期扩展、合规风险

**开场 prompt（直接复制）**：

```
当前阶段：项目立项，只做产品讨论和立项设计，暂时不要写代码。
你的角色：我的产品合伙人。

我想做的是：[一句话描述]
面向用户：[谁]
解决什么问题：[什么]
参考产品：[链接 / 截图，可选]

请按以下方式追问我：
1. 围绕项目类型、目标用户、核心问题逐项深挖
2. 主动指出我没想到的盲区（权限、数据结构、状态字段、后期扩展、合规风险、性能瓶颈、跨端兼容、安全边界）
3. 帮我判断第一版 MVP 该做什么、不该做什么
4. 给唯一推荐，不要列多方案让我选
5. 讨论清楚后告诉我"现在可以写立项文档了"，等我确认再写
```

**追问 prompt（AI 偷懒时用）**：

```
不要只回答我问的，要补我没问的。
特别是：[挑当前没聊到的盲区，比如"权限模型""数据归属""状态机"]——这三块你怎么看？给判断。
```

### 1.4 📝 沉淀落地期（AI 起草 + 你确认）

#### 1.4.1 功能清单拆解

讨论清楚后，先让 AI 输出详细的功能清单（不是直接写立项文档）：

```
请根据立项文档输出一份详细的功能清单，写入 docs/b-功能清单.md。

拆解要求：
1. 按三个维度交叉拆解，不要只按模块拆：
   - 按角色：先列有哪些角色（普通用户、商家、管理员等），再看每个角色能做什么
   - 按业务流程：顺着一件事走（如下单→支付→发货→确认收货）
   - 按模块：把相关功能归到一起（如账号模块、商品模块、订单模块）

2. 每个功能必须说清 6 件事：
   - 给谁用
   - 需要哪些输入
   - 产生什么结果
   - 涉及哪些角色和权限
   - 正常流程是什么
   - 有哪些异常和边界情况

3. 复杂功能拆解与关联（重要）：
   - 如果某个功能内部藏着一整套规则（如角色权限系统、订单流转、支付退款、优惠叠加），
     把它单独拆成一份文档（如 docs/b-功能-订单流转.md，沿用 `b-` 前缀表示属于功能清单），不要硬塞进功能清单
   - 功能清单里对应位置留一个索引指向这份文档：→ 详见 [b-功能-订单流转.md](./b-功能-订单流转.md)
   - 拆出去的文档开头写清楚它属于功能清单的哪个模块，保持双向关联
   - 判断标准：拿不准的，觉得难的，直接拆出来

4. 输出格式：
   - 按模块分组，每个模块下列功能
   - 每个功能给一个编号（如 F-ORD-001），方便后续阶段计划引用
   - 复杂功能索引用 → 标记

先不要写代码。
```

**功能拆解的三个维度**：
1. **按角色拆解**：先列清楚有哪些角色（普通用户、商家、管理员等），再看每个角色各自能做什么
2. **按业务流程拆解**：顺着一件事的流程走（如下单、支付、发货、确认收货等）
3. **按模块拆解**：把相关功能归到一起（如账号模块、商品模块、订单模块）

**复杂功能特殊处理**：
- 功能要写清楚，否则 AI 自行脑补，可能出现边界问题
- 可以和 AI 一边对功能，一边让它把功能清单写成文档，哪个功能有问题，就直接在文档上改
- 高复杂功能（如订单流转、支付退款、优惠叠加、角色权限系统）写一份专门的文档，再到功能清单里面留一个索引指过去
- 拆出去的文档和功能清单保持双向关联：清单里留索引指向子文档，子文档开头标明所属模块
- 如果拿不准这个标准，将觉得难的直接拆出来

#### 1.4.2 阶段计划

功能清单确认后，让 AI 输出阶段计划：

```
请根据功能清单，把开发拆成几个大阶段：
- 第一阶段必须是一个能跑通核心流程的最小可用版本（MVP）
- 请说明每个阶段包含哪些功能
- 为什么这么排

先只给大的阶段划分，不要现在就细化到具体技术实现和详细开发计划。
```

**阶段计划原则**：
- 阶段先定大方向，细节往后放（这个时候还不确定开发框架等等细节，等确定了再去规划细节）
- 第一阶段必须是 MVP（能跑通核心流程的最小可用版本）

#### 1.4.3 文档组织

**文档组织原则**：
- 一份文档只做一件事
- 文档之间的引用关系要像线一样清楚
- 功能清单根据立项文档来
- 复杂功能从功能清单索引出去
- 阶段计划又是基于功能清单来排

#### 1.4.4 立项文档生成

讨论清楚后，**让 AI 一次性生成**立项文档（不要边写边改）：

```
现在请把刚才的讨论结果写入项目根目录的 docs/a-立项文档.md。
必须包含 8 项内容：
1. 项目立项说明（是什么 / 面向谁 / 解决什么问题 / 为什么值得做 / 第一版核心目标）
2. 项目核心功能和边界（第一版做什么、不做什么）
3. 产品流程和用户路径（页面怎么走、功能怎么串、数据怎么流动）
4. 数据和业务对象（用户、订单、任务、文章、商品、文件、评论、权限等）
5. 技术路线初步判断（网页 / 小程序 / App / Web MVP）
6. 项目爆点和差异点
7. 后期规划和里程碑（第一版、第二版、长期版本）
8. 验收规则（第一版做到什么程度才算完成——必须有可检查标准）

生成后让我 review，我确认后再提交。
```

### 1.5 ⚡ 执行交付期

立项阶段**没有代码执行**。唯一的"执行"是：
- 创建空项目文件夹（命名规范：英文 + 中划线）
- 在 AI IDE 里打开

### 1.6 ✅ 验收固化期

验收清单：
- [ ] 项目文件夹存在且命名规范
- [ ] 立项文档已落盘，覆盖 8 项内容
- [ ] **功能清单已拆解**（三维度：角色 / 业务流程 / 模块）
- [ ] **复杂功能已单独拆文档**（订单流转 / 支付退款 / 优惠叠加 / 权限系统）
- [ ] **阶段计划已制定**（第一阶段是 MVP，只给大方向不细化到技术实现）
- [ ] AI 明确说过"暂时不要写代码"
- [ ] 已 Git 提交：`chore: 完成项目立项`

### 1.7 📄 文档策略
**一次性生成**。立项文档是后续所有阶段的"宪法源"，不要边做边改。只有大方向变更时才修订，且必须重新走一遍挖掘期。

### 1.8 🚫 反模式
- ❌ 直接说"帮我做个项目"——AI 自由发挥
- ❌ 跳过讨论直接让 AI 创建文件、装依赖
- ❌ 立项文档只在对话里聊过没落盘
- ❌ 边写代码边补立项文档——本末倒置
- ❌ **跳过功能清单直接写代码**——AI 自由脑补边界
- ❌ **功能清单只按模块拆**——漏掉角色和业务流程维度
- ❌ **复杂功能硬塞进功能清单**——应单独拆文档 + 留索引
- ❌ **没做阶段计划就开始写代码**——不知道先做什么，MVP 跑不通
- ❌ **阶段计划直接细化到技术实现**——选型没定排细节 = 白排

---

## 第 2 章：阶段 2 — 技术选型

### 2.1 阶段目标
把技术路线钉死，不让后续摇摆。给唯一推荐，不要列多方案。

### 2.2 ⏱️ 预计工作量
1-2 轮对话，1 次文档落地，1 次 Git 提交。

### 2.3 🗣️ 对话挖掘期

**核心挖掘点**：
- 产品形态（Web / 小程序 / App / 后台）
- 部署要求（私有 / 公有 / 本地）
- 维护能力（1人 / 小团队 / 中型团队）
- 后期可能的扩展方向（影响选型弹性）
- 团队已有技术栈（优先复用，不为了新项目学新东西）

**开场 prompt**：

```
这是我的立项文档：[贴文档]
产品形态：[Web / 小程序 / App / 后台]
部署要求：[私有 / 公有 / 本地]
维护能力：[1人 / 小团队 / 中型团队]
已有技术栈偏好：[如果有，写出来]

请给出唯一推荐技术栈，不要列多个方案。说明：
- 前端框架 + UI 组件库
- 后端语言 + 框架
- 数据库
- 主要框架和 SDK
- 为什么选它们（结合我的项目情况，不要泛泛而谈）
- 为什么不选其他常见方案（至少列 3 个被淘汰方案及理由）
- 哪些情况才需要重新评估技术路线
```

**追问 prompt（AI 想甩锅时用）**：

```
你列了 A/B/C 三套让我选——这是甩锅。请基于我的项目情况给唯一推荐，并说明为什么不选另外两套。
如果三套真的差不多，说明选型维度不够，请补充维度后重新判断。
```

### 2.4 📝 沉淀落地期

```
请把刚才的选型结论写入 docs/d-选型.md。
结构：前端 / 后端 / 数据库 / SDK / 选的理由 / 不选其他方案的理由 / 重评估条件。
不要在文档里留"建议方案"或"可选方案"——只写最终决定。
```

### 2.5 ⚡ 执行交付期

本阶段无代码执行。但可以让 AI 验证选型可行性：

```
请验证这套技术栈的可行性：
1. 各组件之间是否兼容（版本是否对得上）
2. 关键依赖是否还在维护（查最新 release 时间）
3. 是否有已知重大问题或替代方案推荐
不要写代码，只给结论。
```

### 2.6 ✅ 验收固化期
- [ ] 技术选型文档已落盘
- [ ] AI 能说清"为什么不选其他方案"
- [ ] 已 Git 提交：`chore: 确定项目技术栈`

### 2.7 📄 文档策略
**一次性生成**。技术栈一旦定下就不轻易改。如需变更，必须重新走挖掘期并更新文档。

### 2.8 🚫 反模式
- ❌ 让 AI 列 3 套方案让你选
- ❌ 不说明"为什么不选其他方案"
- ❌ 选型文档只写选什么，不写不选什么

---

## 第 3 章：阶段 3 — 架构设计文档

### 3.1 阶段目标
技术选型后，先定清楚项目代码怎么组织、目录怎么放。这是 Agent 宪法和实施真源文档的基础。

### 3.2 三份架构文档的关系（先搞清再动手）

本项目有三份架构文档，各管不同的事，不要混为一谈：

| 文档 | 生成阶段 | 回答的问题 | 比喻 |
|---|---|---|---|
| **架构设计文档**（本阶段生成，1 份） | 第 3 章 | "规则是什么" | 建筑设计图 |
| **架构实施文档**（搭骨架时生成，前端+后端各 1 份） | 第 6 章 / 第 8 章 | "具体怎么做" | 施工方案 |
| **架构实施真源文档**（验收时反推，前端+后端各 1 份） | 第 10 章 | "实际怎么做的" | 竣工图 |

**为什么要三份而不是一份？**

1. **写的时间点不同**：设计规则在写代码前定，实施计划在搭骨架时定，实际实现在验收时反推
2. **用途不同**：设计规则是约束，实施计划是施工图，真源文档是验收依据
3. **变化频率不同**：设计规则很少变，实施计划偶尔变，真源文档随代码变
4. **如果合一份**：设计规则被实施细节淹没，真源文档和设计意图混淆，分不清"应该怎样"和"实际怎样"

**三者关系图**：

```
设计规则                实施计划                实际实现
（应该怎样）    →    （具体怎么做）    →    （实际怎么做的）
 第3章              第6章/第8章              第10章反推

架构设计文档    →    前端架构实施文档    →    前端架构实施真源文档
                    后端架构实施文档    →    后端架构实施真源文档
```

> **简单记**：第 3 章定规则 → 第 6/8 章定方案 → 第 10 章验实际。规则变了先改第 3 章，方案变了改第 6/8 章，验收后真源文档反映代码实际状态。

### 3.3 ⏱️ 预计工作量
1-2 轮对话，1 次文档落地，1 次 Git 提交。

### 3.4 🗣️ 对话挖掘期

**核心挖掘点**：
- 目录结构规则（每个目录的职责边界，放什么、不放什么）
- 接口响应规则（统一格式、错误码、分页）
- 错误处理规则（异常捕获、日志、返回）
- 日志规则（级别、格式、关键操作必记）
- 数据库连接规则（连接池、事务边界）
- 权限入口（中间件 / 装饰器 / 注解）
- 框架复用边界（哪些必须用框架、哪些可自封装）
- 新增模块规则（一个新模块要改哪些地方）

**开场 prompt**：

```
当前阶段：架构设计文档。暂时不要写代码。
你的角色：项目架构顾问。

上下文：
- 立项文档：[贴]
- 技术选型文档：[贴]

请输出《项目架构设计文档》草稿，包含：
1. 目录结构规则（每个目录职责 / 放什么 / 不放什么）
2. 接口响应规则（统一格式 / 错误码表 / 分页规则）
3. 错误处理规则（异常捕获 / 日志记录 / 返回格式）
4. 日志规则（级别 / 格式 / 关键操作清单）
5. 数据库连接规则（连接池 / 事务边界 / ORM 使用规范）
6. 权限入口（认证中间件 / 角色校验 / 数据归属校验位置）
7. 框架复用边界（必须用框架 / 可封装 / 禁止自造）

先列大纲让我确认，确认后再写完整版。
```

### 3.5 📝 沉淀落地期

```
请写入 docs/e-架构设计文档.md。
这是后续 Agent 宪法和实施真源文档的基础，目录结构 / 接口规则 / 错误处理等变更时必须先改这份文档。
```

### 3.6 ✅ 验收固化期
- [ ] 架构设计文档已落盘
- [ ] 目录结构规则清晰（每个目录职责明确）
- [ ] 接口响应规则（统一格式 + 错误码表）
- [ ] 已 Git 提交：`chore: 完成架构设计文档`

### 3.7 🚫 反模式
- ❌ 跳过架构设计文档直接写宪法
- ❌ 把长期原则塞进架构设计文档（那是宪法的职责）
- ❌ 把具体实施细节塞进架构设计文档（那是第 6/8 章实施文档的职责）

---

## 第 4 章：阶段 4 — Agent 宪法（一点点攒）

### 4.1 阶段目标
给 AI 立长期规矩，让它在后续每个阶段稳定不漂移。**宪法不是一次性写完的，是这个项目定一条规矩就一点点加。**

### 4.2 ⏱️ 预计工作量
1-2 轮对话写基础条款，后续每个阶段/踩坑时持续追加。1 次初始 Git 提交。

### 4.3 🗣️ 对话挖掘期

**核心挖掘点**：
- 项目专属规则（命名、错误码、返回格式）
- 禁止事项（不要做什么）
- 验收标准（每轮交付的硬要求）
- 哪些必须用框架能力、不允许自造

**开场 prompt**：

```
当前阶段：Agent 宪法创建。暂时不要写业务代码。
你的角色：项目架构顾问。

上下文：
- 立项文档：[贴]
- 技术选型文档：[贴]
- 架构设计文档：[贴]

请根据当前项目的真实情况，深度结合以上文档，起草一份专属于本项目的 AGENT_CONSTITUTION.md。
要求：
1. 短：不超过 200 行，AI 能一次读完
2. 硬：每条都是必须遵守的硬规则，不写"建议""尽量"
3. 清楚：每条能直接判断对错
4. 只写长期原则，不写具体技术细节（目录结构、字段命名等放架构设计文档 / 真源文档）
5. 必须包含：项目专属条款 / 禁止事项 / 验收标准 / 框架优先原则
6. 必须结合本项目实际用的语言、框架、业务，把通用规则落到项目上

先列大纲让我确认，确认后再写基础版。
后续每踩一个坑、每定一个规则，就往宪法里加一条。
```

> **关键**：关键词是"结合项目的真实情况"。立项、功能清单、技术栈都是模板里没有的，AI 必须根据你实际用的语言、框架、业务，把通用规则落到项目上，这份宪法才真正管用。

**追问 prompt**：

```
请检查每条规则：
1. 这条是长期的吗？只在当前阶段有效的不要写进来
2. 这条能直接判断对错吗？模糊的请改写或删除
3. 这条和架构设计文档会不会重复？重复的请移到架构设计文档
4. 项目专属条款够不够？至少补 5 条本项目特有的约束
```

**定制完成后，看不懂别装懂**：

```
请用大白话给我讲清楚每一条到底在管什么。
有我看不懂的地方，不要用技术名词糊弄，用最直观的语言重讲一遍。
```

### 4.4 📝 沉淀落地期

```
请写入 AGENT_CONSTITUTION.md。

同时，请在当前 AI 工具会自动读取的位置，帮我创建入口文件并引用宪法。
入口文件只写一行引用，不复制内容（避免多处不同步）。
```

> **宪法放置位置**：不同工具要求的位置和文件名不一样（如 AGENTS.md / CLAUDE.md / .trae/rules/project.md）。最稳的办法是直接问 AI 工具"请在当前工具会自动读取的位置帮我创建"，它会帮你放到对的地方。放对了，工具每次都自动读；放错了，工具根本不读，宪法等于白写。

### 4.5 ⚡ 执行交付期

无业务代码。仅创建配置文件。

### 4.6 ✅ 验收固化期
- [ ] 宪法基础条款不超过 200 行
- [ ] 含项目专属条款 ≥ 5 条
- [ ] 含禁止事项、验收标准
- [ ] 宪法已放在 AI 工具自动读取的位置
- [ ] 入口文件已创建并引用宪法（不复制内容）
- [ ] **每条规则你都能用大白话讲清楚**（看不懂的已让 AI 解释）
- [ ] 已 Git 提交：`chore: 创建项目 Agent 宪法（基础版）`

### 4.7 📄 文档策略
**一点点攒 + 长期不变**。先写基础条款，后续每踩一个坑、每定一个规则就加一条。宪法是稳定锚点，规则细则放架构设计文档和真源文档。

### 4.8 🚫 反模式
- ❌ 试图一次性写完所有规则（应该一点点攒）
- ❌ 把所有技术细节、目录结构塞进宪法（放架构设计文档）
- ❌ 写"建议""尽量"这种软规则
- ❌ 入口文件复制宪法内容（多处会不同步）
- ❌ **拿到通用模板直接复制粘贴**（必须让 AI 结合项目真实情况定制，通用模板里没有你的立项、功能清单、技术栈）
- ❌ **宪法放错位置**（放对了工具才自动读，放错了等于白写）
- ❌ 看不懂的规则不问就放过（看不懂 = 以后管不住 AI）

---

## 第 5 章：阶段 5 — 实施真源文档拆解

### 5.1 阶段目标
把项目拆成可执行、可验收的阶段和步骤。**不是越细越好，三大核心：明确执行目标、划定任务边界、制定验收标准。**

### 5.2 ⏱️ 预计工作量
1-2 轮对话拆首个阶段，后续每完成一个阶段拆下一个。1 次初始 Git 提交。

### 5.3 🗣️ 对话挖掘期

**核心挖掘点**：
- 大阶段划分（基于功能清单和阶段计划）
- 每个大阶段的子阶段
- 每个子阶段的步骤
- 每步的验收标准

**开场 prompt**：

```
当前阶段：实施真源文档拆解。暂时不要写代码。
你的角色：项目实施规划师。

上下文：
- 立项文档：[贴]
- 功能清单：[贴]
- 阶段计划：[贴]
- Agent 宪法：[贴]

请先把首个大阶段拆解成实施真源文档：
1. 大阶段名称和目标
2. 子阶段划分（每个子阶段的目标）
3. 每个子阶段的步骤（每步做什么）
4. 每步的验收标准（怎么算做完）

三大核心：明确执行目标、划定任务边界、制定验收标准。
不要越细越好——能说清目标、边界、验收就行。

先只拆首个阶段，后续完成后再拆下一个。避免一次性太细导致后续大幅修改。
```

### 5.4 📝 沉淀落地期

```
请写入 docs/f-实施真源文档-第N阶段.md。
这是这个大阶段的具体实施依据，开发与验收都以这个文档为主。
如果有临时想法，先写回真源文档里，不要直接改代码。
```

> **拆完之后你一定要看。** 这份文档不光是给 AI 执行的时候看的，它也是你开写代码前，最后一次和 AI 对需求的机会。你要确认 AI 写出来的实施文档，和你的需求有没有偏差。有偏差就先讨论、改文档。看不懂就让它用大白话一条条解释。一定不要怕麻烦。

**需求对照检查 prompt（拆完后用）**：

```
这是我的原始需求：[贴]

请对照实施真源文档检查：
1. 有没有和我需求冲突的地方？
2. 有没有需要先讨论的问题？
3. 有没有遗漏的功能点？
4. 有没有我没想到但应该提前确认的盲区？

如果有，列出来，先别写代码。
```

**任务拆解原则**：
- 完成首个阶段，然后拆解后续阶段，不需要一次性太细
- 业务细节可以随手调整，但地基（底层架构）不能随手翻
- 能改的只是和底层架构无关的需求
- 判断不了就直接问 AI 讨论

### 5.5 ✅ 验收固化期
- [ ] 首个阶段的实施真源文档已落盘
- [ ] 大阶段 → 子阶段 → 步骤清晰
- [ ] 每步有验收标准
- [ ] **你已 review 过文档，确认和需求没有偏差**
- [ ] 看不懂的地方已让 AI 用大白话解释
- [ ] 已 Git 提交：`chore: 完成首阶段实施真源文档拆解`

### 5.6 🚫 反模式
- ❌ 一次性把所有阶段都拆完（应该完成一个拆一个）
- ❌ 拆得太细（能说清目标、边界、验收就行）
- ❌ 没有验收标准（拆了等于没拆）
- ❌ 临时想法直接改代码不写回真源文档
- ❌ **拆完不看就直接让 AI 执行**（这是你开写前最后一次对需求的机会，不看 = 盲飞）
- ❌ **一句话甩给 AI 让它自己拆**（AI 是真的会偷懒，一句话甩过去它会脑补出一堆你不要的东西，必须给足够上下文和约束）

---

## 第 6 章：阶段 6 — 前端骨架

### 6.1 阶段目标
在写页面前把统一规则定下来：设计 token、组件库、目录规则。本轮只搭最小可运行骨架。

### 6.2 ⏱️ 预计工作量
2-3 轮对话，1 次文档落地，1-2 次代码提交。

### 6.3 🗣️ 对话挖掘期

**核心挖掘点**：
- 设计 token（颜色、字号、间距、圆角、阴影）—— 这决定后续所有页面一致性
- UI 组件库选型（结合技术选型，不要重复造轮子）
- 目录结构规则（pages / components / api / utils / store 怎么分）
- 路由结构（基于文件还是配置）
- 状态管理方案（什么时候用全局、什么时候用局部）
- 接口层封装规则（请求拦截、错误处理、loading）
- 多端适配策略（如果涉及小程序 / H5 / App）

**开场 prompt**：

```
当前阶段：前端骨架搭建。本轮只搭最小可运行骨架，不写业务页面。
你的角色：前端架构顾问。

上下文：
- 立项文档：[贴]
- 技术选型文档：[贴]
- 架构设计文档：[贴]
- Agent 宪法：[贴]

请先输出《前端架构实施计划》，包含：
1. 设计 token 定义（颜色 / 字号 / 间距 / 圆角 / 阴影，给具体值）
2. UI 组件库选择（结合技术栈，给唯一推荐）
3. 目录结构规则（每个目录的职责边界）
4. 路由方案（基于文件 / 配置，理由）
5. 状态管理方案（全局 / 局部边界）
6. 接口层封装规则（请求 / 响应 / 错误 / loading）
7. 多端适配策略（如适用）
8. 本轮交付清单（最小可运行：能启动 / 首页能打开 / token 已定 / 组件库接入）
9. 本轮不做什么

我确认计划后再开始写代码。
```

**追问 prompt**：

```
设计 token 还缺：暗色模式 / 响应式断点 / 字号阶梯（h1-h6 + body + caption）。
目录结构请补：每个目录放什么、不放什么、命名规则。
接口层请补：错误码统一处理规则、token 自动续期、超时重试策略。
```

### 6.4 📝 沉淀落地期

```
请把刚才的计划写入 docs/g-前端架构实施文档.md。
后续每次规则变更都要同步这份文档。
```

### 6.5 ⚡ 执行交付期（AI 主导）

文档确认后，可以让 AI 直接执行：

```
现在按文档搭骨架。本轮范围锁死：
- 项目能启动
- 首页能打开（一个空白页或占位页即可）
- 设计 token 已定义并生效
- UI 组件库已接入（能用一个 Button 验证）
- 目录结构按文档创建（空目录也建出来，带 .gitkeep）
- 路由结构搭好（首页路由 + 一个示例路由）
- 接口层封装好（写一个示例请求验证）

不要写业务页面。不要一次性做完所有功能。
完成后按验收清单自检，每项给证据。
```

### 6.6 ✅ 验收固化期
- [ ] 项目能启动（给启动命令和成功输出）
- [ ] 首页能打开（给浏览器截图或 URL）
- [ ] 设计 token 已定义（给文件路径 + 关键变量列表）
- [ ] UI 组件库接入（给一个使用示例）
- [ ] 目录结构符合文档（给目录树）
- [ ] 路由结构搭好（给路由配置文件）
- [ ] 接口层封装完成（给示例请求代码）
- [ ] 已 Git 提交：`feat: 搭建前端最小可运行骨架`
- [ ] 同步前端架构实施文档（如有调整）

### 6.7 📄 文档策略
**先文档后代码 + 边做边补**。骨架搭建过程中如果发现文档没覆盖的细节（比如多端适配的具体规则），改完代码立即补进文档。

### 6.8 🚫 反模式
- ❌ 一次性做完所有页面
- ❌ 到处手写组件不接入 UI 库
- ❌ 设计 token 没定就开始写页面（后续改不动）
- ❌ 接口层每个页面单独写（不复用封装）

---

## 第 7 章：阶段 7 — 数据库设计

### 7.1 阶段目标
先设计文件再建表，不直接连库建表。三大范式必校验。

### 7.2 ⏱️ 预计工作量
2-3 轮对话，1 次文档落地，1 次代码提交。

### 7.3 🗣️ 对话挖掘期

**核心挖掘点**：
- 业务对象清单（用户、订单、任务、文章……）
- 对象关系（1:1 / 1:N / N:N）
- 核心字段（每个对象的字段 + 类型 + 约束）
- 状态字段定义（订单状态、用户状态等枚举值）
- 软删除策略（哪些表软删、字段名、默认值）
- 时间字段规则（created_at / updated_at / deleted_at）
- 密码 / 敏感数据哈希策略
- 金额字段类型（必须 decimal，禁止 float）
- 索引策略（哪些字段常查询、常关联）
- 后期扩展预留（分表 / 分库 / 多租户）

**开场 prompt**：

```
当前阶段：数据库设计。先出设计文件，确认后再建表。
你的角色：数据库架构师。

上下文：
- 立项文档：[贴业务对象部分]
- 技术选型文档：[贴数据库部分]
- 架构设计文档：[贴数据相关部分]
- Agent 宪法：[贴数据相关条款]

请按以下顺序推进，每步等我确认再进下一步：

第 1 步：业务对象清单 + 关系图
- 列出所有业务对象
- 标注对象间关系（1:1 / 1:N / N:N）
- 用 Mermaid ER 图可视化

第 2 步：核心表设计
- 每张表的字段（名称 / 类型 / 约束 / 默认值 / 注释）
- 主键 / 外键 / 唯一索引 / 普通索引
- 状态字段的枚举值
- 软删除字段
- 时间字段

第 3 步：三大范式校验
- 1NF：字段原子性
- 2NF：消除部分依赖
- 3NF：消除传递依赖
- 反范式设计需说明理由 + 同步策略

第 4 步：生成 schema.sql / migration 文件
- 不要直接连库建表
- 文件路径：db/schema.sql 或 migrations/

每步给完让我 review，确认后再进下一步。
```

**追问 prompt**：

```
请主动检查盲区：
1. 金额字段是不是 decimal？（不是就改）
2. 密码字段是不是哈希存储？（不能明文）
3. 软删除字段是否一致（统一 deleted_at，不要混用 is_deleted）
4. 状态字段枚举值是否完整（覆盖所有业务状态）
5. 索引是否覆盖高频查询（外键、状态、时间）
6. 是否有 N:N 关系漏了中间表
7. 时间字段时区策略
8. 字符集是否统一 utf8mb4
```

### 7.4 📝 沉淀落地期

```
请把设计结果写入 docs/h-数据库设计文档.md，并生成 db/schema.sql。
文档结构：业务对象 / 关系图 / 核心表 / 字段规范 / 软删除策略 / 三大范式校验 / 索引策略。
schema.sql 必须可独立执行（不依赖手动操作）。
```

### 7.5 ⚡ 执行交付期

```
现在执行 schema.sql 建表。
本轮范围：能连接数据库 / 能建表 / 能插入和查询测试数据。
不要写业务逻辑。不要改前端。
完成后给：
- 连接证据（命令 + 输出）
- 建表证据（SHOW TABLES 输出）
- 测试数据证据（INSERT + SELECT 输出）
```

### 7.6 ✅ 验收固化期
- [ ] schema.sql 文件已落盘且可独立执行
- [ ] 数据库设计文档已落盘
- [ ] 三大范式校验通过（反范式有理由 + 同步策略）
- [ ] 金额字段用 decimal
- [ ] 密码字段哈希存储
- [ ] 软删除字段一致
- [ ] 能连接 / 能建表 / 能插入查询
- [ ] 已 Git 提交：`feat: 完成数据库设计文件并跑通基础结构`
- [ ] 同步数据库设计文档

### 7.7 📄 文档策略
**先文档后代码**。schema 文件本身就是文档的一部分。后续每次结构变更必须先改 schema 文件 + 设计文档，再执行迁移。

### 7.8 🚫 反模式
- ❌ 直接连库建表（不可回溯）
- ❌ 密码明文存储
- ❌ 金额用 float
- ❌ 状态字段用数字不注释含义
- ❌ 跳过范式校验

---

## 第 8 章：阶段 8 — 后端骨架

### 8.1 阶段目标
基于架构设计文档搭骨架，跑通启动线 / 接口线 / 业务线 / 运维线四条线。同时生成《后端架构实施文档》，把第 3 章的规则细化为可执行的实施细节。

### 8.2 ⏱️ 预计工作量
3-4 轮对话，1 次文档落地，1-2 次代码提交。

### 8.3 🗣️ 对话挖掘期

**核心挖掘点**：
- 架构设计文档里的规则怎么落地为具体实现
- 4 条线怎么验证跑通
- 配置 / 环境变量规范处理
- 目录结构的具体职责划分

**开场 prompt**：

```
当前阶段：后端骨架搭建。先出实施计划，确认后再写代码。
你的角色：后端架构师。

上下文：
- 立项文档：[贴]
- 技术选型文档：[贴]
- 架构设计文档：[贴]
- 数据库设计文档：[贴]
- Agent 宪法：[贴]

请基于架构设计文档，输出《后端架构实施计划》，包含：
1. 目录结构（每个目录职责 / 放什么 / 不放什么 / 示例文件）
2. 接口响应规则（统一 JSON 格式 / 错误码表 / 分页格式 + 示例）
3. 错误处理规则（异常类层级 / 捕获流程 / 返回格式 + 示例）
4. 日志规则（级别 / 格式 / 关键操作清单 / 配置方式）
5. 数据库连接（连接池配置 / ORM 使用规范 / 事务边界）
6. 权限入口（认证中间件 / 角色校验 / 数据归属校验位置）
7. 框架复用边界（哪些用框架 / 哪些自封装 / 禁止自造清单）
8. 配置规范（.env 结构 / 环境变量清单 / 配置文件层级）
9. 本轮交付清单（4 条线跑通：启动 / 接口 / 业务 / 运维）
10. 本轮不做什么

我确认计划后再开始写代码。
```

**追问 prompt**：

```
接口响应请补：成功 / 失败 / 分页 / 批量操作 4 种格式的完整示例。
错误处理请补：业务异常 / 系统异常 / 第三方异常 分别怎么处理、怎么返回。
日志请补：每个关键操作的日志模板（入参摘要 / 出参摘要 / 耗时 / 异常堆栈）。
配置请补：开发 / 测试 / 生产三套环境的差异点。
```

### 8.4 📝 沉淀落地期

```
请把实施计划写入 docs/i-后端架构实施文档.md。
这是后端骨架的实施依据，后续每次规则变更都要同步这份文档。
第 3 章架构设计文档是规则总纲，本文档是落地细节，两者不重复但保持一致。
```

> **文档分工**：第 3 章《架构设计文档》= 规则层面（"应该怎样"），第 8 章《后端架构实施文档》= 实施层面（"具体怎么做"）。如果实施过程中发现规则缺失，先补第 3 章再补第 8 章。

### 8.5 ⚡ 执行交付期

```
现在按后端架构实施文档搭骨架。本轮范围锁死：
- 项目能启动（启动命令 + 成功输出）
- 至少 1 个示例接口能跑通（健康检查接口）
- 4 条线跑通：启动线 / 接口线 / 业务线（示例接口）/ 运维线（日志 / 配置）
- 目录结构按实施文档创建
- 配置文件 / 环境变量按规范处理
- 数据库连接验证

不要写完整业务。不要一次性做完所有接口。
完成后按验收清单自检，每项给证据。
```

### 8.6 ✅ 验收固化期
- [ ] 《后端架构实施文档》已落盘
- [ ] 4 条线跑通（启动 / 接口 / 业务 / 运维，每条给证据）
- [ ] 目录结构符合实施文档
- [ ] 至少 1 个示例接口能调通
- [ ] 配置 / 环境变量规范
- [ ] 日志能输出
- [ ] 已 Git 提交：`feat: 搭建后端最小可运行骨架，落盘后端架构实施文档`
- [ ] 同步架构设计文档和后端架构实施文档（如有调整）

### 8.7 📄 文档策略
**先文档后代码 + 边做边补**。后端架构实施文档是骨架搭建的依据，搭建过程中如发现规则缺失，立即补文档。验收阶段把"实际怎么实现的"反推成《后端架构实施真源文档》（见第 10 章）。

### 8.8 🚫 反模式
- ❌ 跳过架构设计文档直接写代码
- ❌ **不生成后端架构实施文档就开始搭骨架**（前端有实施文档，后端也要对齐）
- ❌ 一上来写完整业务（必须先骨架）
- ❌ 4 条线没跑通就宣布完成
- ❌ 配置硬编码（不用环境变量）
- ❌ 后端架构实施文档和架构设计文档内容重复（前者是落地细节，后者是规则总纲）

---

## 第 9 章：阶段 9 — 后端联调

### 9.1 阶段目标
前后端骨架都跑通后，验证前后端是否接得上。这是业务模块开发前的最后一道基础验证。

### 9.2 ⏱️ 预计工作量
1-2 轮对话，1 次代码提交。

### 9.3 🗣️ 对话挖掘期

**核心挖掘点**：
- 前端接口层封装是否能调通后端示例接口
- 跨域配置是否正确
- 响应格式前后端是否一致
- 错误码处理是否对齐

**开场 prompt**：

```
当前阶段：后端联调。验证前后端骨架是否接得上。
你的角色：全栈联调工程师。

上下文：
- 前端架构实施文档：[贴]
- 后端架构实施文档：[贴]
- Agent 宪法：[贴]

请完成前后端联调验证：
1. 前端调用后端示例接口（健康检查），返回正常
2. 跨域配置正确（前端能访问后端）
3. 响应格式前后端一致（前端解析后端返回无问题）
4. 错误场景验证（接口报错时前端能正确捕获）
5. 网络/超时场景验证

每项给：请求命令 + 响应输出 + 是否通过。
不要写业务功能。只验证骨架联调。
```

### 9.4 ✅ 验收固化期
- [ ] 前端能调通后端示例接口
- [ ] 跨域配置正确
- [ ] 响应格式前后端一致
- [ ] 错误场景验证通过
- [ ] 已 Git 提交：`feat: 前后端联调验证通过`

### 9.5 🚫 反模式
- ❌ 跳过联调直接写业务（接口接不上再查更难）
- ❌ 只验证成功场景不验证错误场景

---

## 第 10 章：阶段 10 — 架构验收（后端 + 前端）

### 10.1 阶段目标
用证据证明规则立住了。不是听 AI 说"已完成"，而是看文件 / 命令 / 输出 / 日志 / 截图。同时生成《后端架构实施真源文档》和《前端架构实施真源文档》。

### 10.2 ⏱️ 预计工作量
3-4 轮对话，2 次文档落地，1-2 次代码提交。

### 10.3 🗣️ 对话挖掘期

**核心挖掘点**：
- 11 种接口场景是否都有示例
- 启动证据包是否完整
- 框架复用边界是否真的遵守（没自造轮子）
- 新增模块规则是否清晰可执行
- 文档和代码是否一致

**开场 prompt**：

```
当前阶段：后端架构验收。本轮不改代码，只验收 + 反推真源文档。
你的角色：后端架构审查员。

请对当前后端做验收。每项必须给证据（文件 / 命令 / 输出 / 日志），未验证项标记"未验证"。
禁止使用"基本完成""应该没问题""理论上可以"。

验收清单：
1. 目录责任：每个目录的职责和实际内容是否一致
2. 接口响应：11 种场景示例是否齐全（成功 / 失败 / 参数错 / 未登录 / 无权限 / 资源不存在 / 资源冲突 / 限流 / 服务器错 / 数据校验失败 / 异步任务）
3. 错误处理：异常捕获 / 日志 / 返回是否符合规则
4. 日志规则：级别 / 格式 / 关键操作是否记录
5. 数据库连接：连接池 / 事务 / ORM 使用是否规范
6. 权限入口：认证 / 角色校验 / 数据归属校验是否到位
7. 框架复用：是否有自造轮子（应该用框架的地方）
8. 新增模块规则：是否清晰可执行（举一个新模块的步骤）
9. 启动证据包：4 条线每条给验证证据

每项给：规则来源（文档条款）+ 实际证据（文件 / 命令）+ 是否通过。
```

### 10.4 📝 沉淀落地期（后端真源文档）

```
请基于验收结果，反推生成《后端架构实施真源文档》docs/k-后端架构实施真源文档.md。
不要改任何代码。文档要反映实际实现，不是设计意图。
每项给文件路径 + 行号作为证据。说不清的标记"未明确"。

文档结构：
1. 语言框架（实际版本）
2. 目录责任表（实际职责 + 文件路径）
3. 接口响应规则（实际格式 + 示例文件）
4. 错误处理（实际位置 + 代码示例）
5. 日志（实际配置 + 输出示例）
6. 数据库连接（实际配置 + 文件）
7. 权限入口（实际中间件 + 文件路径）
8. 框架复用边界（实际用了哪些框架能力）
9. 新增模块规则（实际要改哪些地方）
10. 启动证据包（实际命令 + 输出）
```

### 10.5 ⚡ 执行交付期

本阶段**不写代码**。只生成验收报告 + 真源文档。

如果验收发现需要整改的问题，转入第 11 章业务模块开发时一起处理，或单独开一轮整改（用 systematic-debugging 流程）。

### 10.6 ✅ 验收固化期
- [ ] 验收报告生成（9 项每项有证据）
- [ ] 11 种接口场景示例齐全
- [ ] 启动证据包完整
- [ ] 《后端架构实施真源文档》已落盘
- [ ] 文档和代码一致（无"未明确"项，或已列入整改）
- [ ] 已 Git 提交：`chore: 后端架构验收通过，固化实施真源文档`

### 10.7 📄 文档策略（后端+前端通用）
**反推生成 + 持续同步**。真源文档是 AI 的"项目记忆"，后续每次规则变更必须立即同步这份文档（改代码前先改文档）。

### 10.8 🚫 反模式（后端验收）
- ❌ 听"已完成"就放过
- ❌ 验收不给证据（只说"已检查"）
- ❌ 把细则塞进 Agent 宪法（应该放真源文档）
- ❌ 真源文档写设计意图不写实际实现

---

> **以下为前端架构验收部分。后端验收完成后，接着验收前端。**
> 执行交付期和文档策略与后端相同（不写代码，只验收+反推真源文档），不再重复。

### 10.9 🗣️ 对话挖掘期（前端验收）

**核心挖掘点**：
- 设计 token 是否定义并生效（颜色 / 字号 / 间距 / 圆角 / 阴影）
- UI 组件库是否接入（不是手写组件）
- 目录结构是否符合前端架构实施文档
- 路由结构是否搭好（首页 + 示例路由）
- 接口层封装是否统一（请求拦截 / 错误处理 / loading）
- 组件复用规则（有没有重复造轮子）
- 状态管理方案是否落地
- 多端适配是否处理（如适用）
- 浏览器实际验证（能打开 / 能交互 / 无控制台报错）

**开场 prompt**：

```
当前阶段：前端架构验收。本轮不改代码，只验收 + 反推真源文档。
你的角色：前端架构审查员。

请对当前前端做验收。每项必须给证据（文件 / 截图 / 命令 / 输出），未验证项标记"未验证"。
禁止使用"基本完成""应该没问题""理论上可以"。

验收清单：
1. 设计 token：颜色 / 字号 / 间距 / 圆角 / 阴影是否定义并生效（给文件路径 + 关键变量）
2. UI 组件库：是否接入，有没有手写组件替代（给使用示例）
3. 目录结构：是否符合前端架构实施文档（给目录树 + 每个目录职责）
4. 路由结构：首页 + 示例路由是否能跑通（给路由配置 + 浏览器截图）
5. 接口层封装：请求拦截 / 错误处理 / loading 是否统一（给封装代码）
6. 组件复用：有没有重复造轮子（对比手写组件 vs UI 库组件）
7. 状态管理：全局 / 局部边界是否清晰（给 store 文件 + 使用示例）
8. 多端适配：如适用，是否处理（给适配方案 + 验证证据）
9. 浏览器验证：首页能打开 / 能交互 / 控制台无报错（给截图）

每项给：规则来源（前端架构实施文档条款）+ 实际证据 + 是否通过。
```

### 10.10 📝 沉淀落地期（前端真源文档）

```
请基于验收结果，反推生成《前端架构实施真源文档》docs/l-前端架构实施真源文档.md。
不要改任何代码。文档要反映实际实现，不是设计意图。
每项给文件路径作为证据。说不清的标记"未明确"。

文档结构：
1. 框架 + UI 库（实际版本）
2. 设计风格 + 设计 token（实际值 + 文件路径）
3. 目录结构（实际职责 + 文件路径）
4. 组件复用规则（哪些用 UI 库 / 哪些自封装）
5. 路由方案（实际配置 + 文件）
6. 接口层封装（实际封装 + 文件路径）
7. 状态管理（实际方案 + 文件路径）
8. 多端适配（实际方案，如适用）
9. 浏览器验证证据包（截图 + URL）
```

### 10.11 ✅ 前端验收固化期
- [ ] 验收报告生成（9 项每项有证据）
- [ ] 设计 token 已定义并生效
- [ ] UI 组件库已接入（无重复造轮子）
- [ ] 目录结构符合文档
- [ ] 浏览器能打开 / 能交互 / 无报错
- [ ] 《前端架构实施真源文档》已落盘
- [ ] 文档和代码一致（无"未明确"项，或已列入整改）
- [ ] 已 Git 提交：`chore: 前端架构验收通过，固化实施真源文档`

### 10.12 🚫 反模式（前端验收）
- ❌ 只看代码不打开浏览器验证
- ❌ 设计 token 定了但没检查是否生效
- ❌ 手写组件替代 UI 库组件不标记
- ❌ 真源文档写设计意图不写实际实现

---

## 第 11 章：阶段 11 — 业务模块开发（多轮）

### 11.1 阶段目标
一个模块一个模块加，每轮交付可验收的小块。**这是项目最长的阶段，也是最容易失控的阶段。**

**开发节奏**：一个模块一个模块写接口，边写边接前端，做完一个模块就验收然后提交 git。

### 11.2 ⏱️ 预计工作量
每个模块 1-3 轮对话，每轮 1 次代码提交。

### 11.3 🗣️ 对话挖掘期（每个模块开始前）

**核心挖掘点**：
- 这个模块涉及哪些业务对象 / 接口 / 页面
- 哪些是 P0（必须）/ P1（重要）/ P2（可选）
- 依赖哪些已完成的模块
- 是否涉及规则变更（如果是，先改真源文档）
- 这个模块的特殊权限 / 数据归属规则

**模块开场 prompt**：

```
当前阶段：业务模块开发 - [模块名]
你的角色：后端开发 + 前端开发

上下文：
- 立项文档：[贴相关部分]
- 实施真源文档：[贴当前阶段]
- 后端架构实施真源文档：[贴]
- 前端架构实施真源文档：[贴]
- 数据库设计文档：[贴]
- Agent 宪法：[贴]
- 已完成模块清单：[列]

本模块要做的接口 / 页面：[列]
依赖的已完成模块：[列]

请先输出本模块实施计划，包含：
1. 接口清单（路径 / 方法 / 入参 / 出参 / 权限 / P0-P2）
2. 页面清单（路由 / 组件 / 状态 / P0-P2）
3. 涉及的业务对象 / 表变更（如有，先改 schema 和数据库设计文档）
4. 涉及的规则变更（如有，先改真源文档）
5. 本轮拆分（一个模块拆 N 轮，每轮做什么）
6. 每轮的验收标准

我确认计划后再开始写。每轮结束前必须走验收清单。
```

### 11.4 📝 沉淀落地期

#### 11.4.1 规则变更同步（动接口规则/错误码/目录约定时）

如果模块涉及规则变更（新增错误码、改响应格式、加权限规则、改目录约定）：

```
在写代码之前，先更新真源文档对应章节：
- [指出改哪份文档、哪一节、加什么内容]
更新完让我确认，确认后再写代码。
```

#### 11.4.2 新增业务功能的文档同步（开发中新增功能时）

业务开发阶段经常会冒出"要不要加个 XX 功能"的想法。**新增功能不是反模式，但"先写代码再补文档"是擦边**。正确做法是文档驱动新增——先改文档，确认后再写代码。

**判断新增功能的影响范围**（动手前先问 AI）：

```
准备新增 [XX 功能]。动手前先告诉我：
1. 这个功能属于哪个模块？要不要新增模块？
2. 影响哪些已有文档：
   - 功能清单（加一条）
   - 实施真源文档（加一节）
   - 数据库设计（要不要加表/字段）
   - 立项文档（要不要更新功能边界/阶段计划）
3. 评估这个改动会不会影响地基（目录结构/核心字段/登录权限/支付流程）
先改文档，我确认后再写代码。
```

**三类新增的文档处理对照表**：

| 新增类型 | 改哪些文档 | 改的时机 |
|---|---|---|
| 新增一个完整模块 | 功能清单加条目 + 实施真源文档加子阶段 + 可能加数据库表 | **写代码前**全部改完 |
| 新增一个接口/页面（已有模块内） | 实施真源文档当前子阶段加一条 + 可能改 schema | **写代码前**改真源文档 |
| 新增一个字段/小调整 | 数据库设计文档/schema 改一条 + 真源文档同步 | **写代码前**改 schema 和文档 |

**关键原则**：
- **先文档后代码**：新增功能前，先让 AI 评估影响哪些文档，改完文档再动手
- **不要"先写后补"**：先写代码再回头补立项文档/功能清单 = 轻微违反"改代码前先改文档"
- **区分"文档同步"和"边写边补立项"**：开发后回填"已实现状态"到文档是正常的文档维护；但"新增功能"必须文档先行，不能写完才补

> 一句话：**新增功能 = 先改文档（功能清单/真源文档/schema）→ 确认 → 再写代码 → 做完回填状态**。不是"边写边补"，是"文档驱动新增"。

### 11.5 ⚡ 执行交付期（每轮）

```
严格遵守 Agent 宪法和实施真源文档，只实施当前子阶段，不要超出范围，做完后停下来汇报，不要自动进入下一个子阶段。

本轮范围锁死（只做这些，不要一次性做完）：
- [任务1]
- [任务2]
- [任务3]

边写边接前端：后端接口写完，立即接前端页面验证。

完成后汇报三件事：
1. 对照实施真源文档，这个阶段哪些做完了，哪些还没做完
2. 到底改了哪些地方，有没有漏做，有没有多做
3. 怎么验证的，验证结果是什么，下一个该做哪个子阶段，为什么

每项给文件 / 命令 / 输出 / 日志作为证据。
最后给明确结论：能否进入下一轮。
```

### 11.6 ✅ 验收固化期（每轮）
- [ ] 本轮范围全部完成
- [ ] 验收清单 7 项全过（功能 / 规则 / 安全 / 数据 / 接口 / 日志 / 回归）
- [ ] 无"基本完成"等模糊词
- [ ] 已 Git 提交：`feat: 完成 XX 模块（含 XX 接口）`
- [ ] 如有规则变更，真源文档已同步
- [ ] 如发现盲点，记入 Skill 反哺清单
- [ ] 更新模块清单模板

### 11.7 📄 文档策略
**先计划后执行 + 边做边同步**。每个模块开始前生成实施计划（可暂存对话里，复杂模块才落盘）。每轮涉及规则变更的，**先改真源文档再改代码**。

### 11.8 🚫 反模式
- ❌ 一口气写完所有业务（必须拆轮）
- ❌ 跳过每轮验收
- ❌ 改了规则不改真源文档
- ❌ 一个模块未验收就开下一个
- ❌ 后端写完不接前端就提交（必须边写边接）

---

## 第 12 章：阶段 12 — 后端安全审查

### 12.1 阶段目标
把 5 道安全关卡逐项交证据。重点查水平越权 + 注入。

### 12.2 ⏱️ 预计工作量
1-2 轮对话，1 次文档落地，1 次代码提交。

### 12.3 🗣️ 对话挖掘期

**核心挖掘点**：
- 配置文件安全（密钥隔离）
- 日志安全（脱敏）
- 依赖安全（漏洞巡检）
- 数据库安全（权限管控）
- 请求层五道关卡（身份 / 权限 / 输入 / 归属 / 注入）

**开场 prompt**：

```
当前阶段：后端安全审查。不要只说"已做安全处理"。
你的角色：安全审查员。

请按以下维度分别说明：
1. 配置文件安全：数据库地址、密钥、token 是否硬编码
2. 日志安全：是否记录敏感数据，如何脱敏
3. 依赖安全：框架/库版本是否有已知漏洞
4. 数据库安全：重要数据的读写删是否有四重校验
5. 请求层五道关卡：身份认证 / 权限控制 / 输入校验 / 数据归属 / 注入防护

每道关卡给：风险 / 处理位置（文件 + 行号）/ 处理规则 / 如何验证 / 是否通过。
看不懂的解释用大白话重讲一遍，不要用技术名词糊弄。
最后输出整改清单，按严重 / 风险 / 建议分级。
```

**追问 prompt（关键，必问）**：

```
重点检查水平越权：
- 列出所有"按 ID 查询 / 修改 / 删除"的接口
- 每个接口验证：用户 A 能不能访问用户 B 的资源？
- 给具体测试用例（用户 A 的 token 调用户 B 的资源 ID，预期 403）

重点检查注入：
- 列出所有"用户输入拼到 SQL / 命令 / 模板"的位置
- 每个位置验证：用的是参数化查询还是字符串拼接？
- 字符串拼接的必须改掉
```

### 12.4 📝 沉淀落地期

```
请把审查结果写入 docs/o-安全审查报告.md。
结构：5 大维度逐项 + 整改清单（按严重度分级）+ 安全边界表 + 权限设计表。
```

### 12.5 ⚡ 执行交付期

本阶段**不改代码**，只审查。整改清单生成后，按严重度分批改：

```
现在按整改清单分批修复：
第 1 批：严重项（必须本轮修完）
- [列]
修完每项给：修复前 / 修复后 / 验证证据。
不要顺手改审查范围外代码。
```

### 12.6 ✅ 验收固化期
- [ ] 5 大维度全过（每项有证据）
- [ ] 无字符串拼接用户输入
- [ ] 水平越权测试通过（给测试用例）
- [ ] 整改清单全部处理完
- [ ] 安全审查报告已落盘
- [ ] 已 Git 提交：`chore: 后端安全审查通过`
- [ ] 如有规则变更，同步真源文档

### 12.7 📄 文档策略
**一次性生成报告 + 整改后追加**。报告生成后，整改结果逐项追加到报告末尾。如发现新的安全规则，同步到真源文档。

### 12.8 🚫 反模式
- ❌ 只做"要不要登录"忽略水平越权
- ❌ 安全说明看不懂（用了技术名词糊弄）
- ❌ 整改时顺手改其他代码
- ❌ 整改后不验证

---

## 第 13 章：阶段 13 — 上线前总验收

### 13.1 阶段目标
上线前最后一次系统检查。5 维度审查 + 风险分级。

### 13.2 ⏱️ 预计工作量
1-2 轮对话，1 次文档落地，1 次代码提交。

### 13.3 🗣️ 对话挖掘期

**核心挖掘点**：
- 结构：目录放置 / 分层 / 重复实现
- 规则：宪法 / 真源文档 / 框架规范
- 安全：输入校验 / 越权 / 密码 / 注入 / 过度防御
- 数据：schema/migration / 范式 / 敏感数据 / 软删除
- 可维护性：命名 / 硬编码 / 提前封装 / 日志 / 错误 / 测试

**开场 prompt**：

```
当前阶段：上线前总验收。本轮不改代码，只审查 + 输出整改清单。
你的角色：代码审查员 + 上线把关人。

请对整个项目做 5 维度审查：

1. 结构：目录放置、分层、重复实现
2. 规则：宪法、真源文档、框架规范是否被违反
3. 安全（涉及后端）：输入校验、越权、密码、注入、过度防御
4. 数据：schema/migration、范式、敏感数据、软删除
5. 可维护性：命名、硬编码、提前封装、日志/错误/测试

每项给"通过 / 风险 / 严重"判断 + 证据（文件 + 行号）。

风险分级：
🔴 严重：阻断合并 / 阻断上线
🟡 风险：建议修，可上线后修
🟢 建议：可选优化

输出整改清单。不擅自重构审查范围外代码。
```

**追问 prompt**：

```
请额外检查：
1. 是否有硬编码（配置 / URL / 密钥 / 错误信息）
2. 是否有 TODO / FIXME 没处理
3. 是否有死代码（未使用的函数 / 文件）
4. 是否有重复实现（多个地方做同一件事）
5. 错误处理是否一致（有的 try-catch 有的没有）
6. 日志是否覆盖关键操作
7. 环境变量是否都从配置读（不硬编码）
```

### 13.4 📝 沉淀落地期

```
请把审查结果写入 docs/p-上线前验收报告.md。
结构：5 维度逐项 + 风险分级清单 + 整改清单 + 上线建议（go / no-go）。
```

### 13.5 ⚡ 执行交付期

按严重度分批改：

```
现在按整改清单修复：
第 1 批：🔴 严重项（必须本轮修完，否则不上线）
- [列]
第 2 批：🟡 风险项（可上线后修，但要在文档里登记）
- [列]

修完每项给：修复前 / 修复后 / 验证证据。
🟢 建议项暂不修，记入后期优化清单。
```

### 13.6 ✅ 验收固化期
- [ ] 5 维度审查完成
- [ ] 无 🔴 严重项
- [ ] 🟡 风险项已登记后期清单
- [ ] 上线前验收报告已落盘
- [ ] 已 Git 提交：`chore: 上线前总验收通过`
- [ ] 同步所有真源文档（最后一次对齐）

### 13.7 📄 文档策略
**一次性生成报告 + 整改后追加**。报告是上线决策依据，必须存档。

### 13.8 🚫 反模式
- ❌ 跳过总验收直接上线
- ❌ 🔴 严重项没修完就上线
- ❌ 审查不给证据
- ❌ 顺手重构审查范围外代码

---

## 第 14 章：阶段 14 — 上线 + Skill 反哺

### 14.1 阶段目标
上线 + 把本次踩的坑反哺到 Skill 库。**反哺是复利的关键，跳过这一步等于白做项目。**

### 14.2 ⏱️ 预计工作量
1 轮对话 + 1 次 Skill 库 commit。

### 14.3 🗣️ 对话挖掘期

**核心挖掘点**：
- 本次项目踩了哪些坑
- 哪些坑是 Skill 没覆盖的
- 哪些 Skill 规则需要强化
- 哪些 Prompt 模板需要补充

**开场 prompt**：

```
当前阶段：项目上线 + Skill 反哺。
你的角色：项目复盘员。

请回顾本次项目，回答：
1. 踩了哪些坑？（列具体场景 + 根因）
2. 哪些坑是现有 Skill / Prompt 没覆盖的？
3. 哪些 Skill 的规则需要强化或补充？
4. 哪些 Prompt 模板需要新增或改进？
5. 下个项目开始前，应该更新哪些文档？

给具体建议：改哪个 Skill 文件、加哪条规则、补哪个 Prompt。
不要泛泛而谈。
```

### 14.4 📝 沉淀落地期

```
请把反哺清单写入 docs/q-项目复盘报告.md。
结构：踩坑清单 / Skill 改进清单 / Prompt 改进清单 / 下次项目提醒。

对于每条新增规则，必须标注来源：
[YYYY-MM-DD] 规则描述 — 来源：XX 项目 XX 阶段 XX 问题
```

### 14.5 ⚡ 执行交付期

```
现在按反哺清单更新 Skill 库：
- [Skill 文件1]：加 [规则]
- [Skill 文件2]：强化 [规则]
- [Prompt 模板]：新增 [模板]

改完给：改了什么 / 为什么改 / 怎么用。
```

### 14.6 ✅ 验收固化期
- [ ] 项目已上线
- [ ] 复盘报告已落盘
- [ ] 至少反哺 1 条新经验到 Skill 库
- [ ] Skill 库已 Git 提交
- [ ] 下次项目提醒已记录

### 14.7 📄 文档策略
**复盘报告一次性生成 + Skill 库持续迭代**。复盘报告是项目存档，Skill 库是复利资产。

### 14.8 🚫 反模式
- ❌ 上线完就结束，Skill 没迭代
- ❌ 反哺只写"以后注意"（要写具体规则）
- ❌ 不记录下次项目提醒

---

## 第 15 章：异常处理流程

### 15.1 改坏了（AI 越补越乱）

```mermaid
flowchart TD
    A[发现 AI 改乱了] --> B[1. 立刻说停]
    B --> C[2. Git 现状盘点]
    C --> D{有 Git 历史吗?}
    D -->|有| E[找到最后一个能跑的提交]
    D -->|没有| F[备份当前代码]
    E --> G[回退到稳定版本]
    F --> G
    G --> H[反推真源文档对齐]
    H --> I[重新开始本轮]
```

**话术（直接复制）**：

```
停。当前改动已经偏离目标。请先列出最近 3 次 Git 提交，告诉我每个版本做了什么。
我决定回退到哪个版本后，再重新开始这一轮。
不要继续补，越补越乱。
```

### 15.2 跑偏了（AI 理解错需求）

```
停。我重新对齐需求：
- 我要的是 [具体描述]
- 你做的是 [AI 当前理解]
- 差异在 [指出具体差异]

请基于我的真实需求重新评估：
1. 当前已做的代码哪些能保留
2. 哪些必须回退
3. 重新做的话计划是什么
不要在错误基础上继续补。
```

### 15.3 文档和代码不一致

```
请检查 [真源文档] 和实际代码是否一致：
1. 列出文档说有但代码没有的规则
2. 列出代码有但文档没写的规则
3. 列出文档和代码描述不一致的规则
每项给文件 + 行号证据。
确认后我决定：是改文档对齐代码，还是改代码对齐文档。
```

### 15.4 AI 用模糊话糊弄

```
禁止使用"基本完成""应该没问题""理论上可以""大概""可能"。
要么"通过"，要么"未通过"加原因。
刚才说的"基本完成"——具体哪些通过、哪些未通过？给清单。
```

### 15.5 AI 列多方案让你选

```
你列了 A/B/C 三套让我选——这是甩锅。
请基于我的项目情况给唯一推荐，并说明为什么不选另外两套。
如果三套真的差不多，说明选型维度不够，请补充维度后重新判断。
```

---

## 第 16 章：文档生成策略详解

### 16.1 七种文档类型及生成时机

| 类型 | 何时生成 | 何时修改 | 修改方式 |
|---|---|---|---|
| **决策型**（立项/选型/宪法） | 讨论清楚后一次性生成 | 大方向变更时 | 重新走挖掘期 |
| **设计型**（架构设计/schema） | 写代码先生成 | 规则变更时 | 先改文档再改代码 |
| **真源型**（实施真源文档） | 验收阶段反推生成 | 每次规则变更立即同步 | 改代码前先改文档 |
| **过程型**（实施计划/验收报告/审查报告） | 每轮/每阶段开始时生成 | 边做边修订 | 整改后追加结果 |
| **反哺型**（Skill / Prompt 库） | 项目结束时反哺 | 持续迭代 | 加规则 / 改规则 / 加模板 |
| **复盘型**（项目复盘报告） | 项目上线后一次性生成 | 不修改 | 存档参考 |
| **ADR 型**（架构决策记录） | 重大技术决策时生成 | 决策被取代时标记废弃 | 新增取代决策的 ADR |

### 16.2 "一次性生成" vs "边做边改" 判断标准

| 信号 | 策略 |
|---|---|
| 内容是稳定的长期决策（方向、选型、宪法） | 一次性生成，少改 |
| 内容是规则定义（架构、数据库设计） | 一次性生成 + 规则变更时改 |
| 内容是实际实现的反映（真源文档） | 反推生成 + 持续同步 |
| 内容是过程记录（计划、报告） | 一次性生成 + 边做边追加 |
| 内容是经验积累（Skill 库） | 持续迭代，每个项目至少改 1 条 |

### 16.3 文档版本管理

- **每个文档独立 Git 提交**：不要把多个文档变更混在一个 commit
- **commit message 说明变更原因**：`docs: 立项文档新增权限模型章节（因 XXX 需求）`
- **重大变更走挖掘期**：不要直接改文档，先讨论清楚再改
- **变更同步代码**：文档改了，代码必须同步；代码改了，文档必须同步

### 16.4 文档失效信号（出现就要立即同步）

- AI 写的代码和文档描述对不上
- 新增的规则只在对话里说过没落盘
- AI 反复问同一个问题（说明文档没写清楚）
- 新模块开发时发现规则有歧义
- 验收时发现"实际实现"和"设计意图"不一致

---

## 第 17 章：跨阶段速查表

### 17.1 阶段 → 模式 → 文档 速查

| 阶段 | 主导模式 | 关键文档 | 文档策略 |
|---|---|---|---|
| 1 立项 | 🗣️ 挖掘 | 立项文档 + 功能清单 + 阶段计划 | 一次性生成 |
| 2 选型 | 🗣️ 挖掘 | 技术选型文档 | 一次性生成 |
| 3 架构设计 | 🗣️ 挖掘 | 架构设计文档 | 一次性生成 + 边做边补 |
| 4 宪法 | 🗣️ 挖掘 | AGENT_CONSTITUTION.md | 一点点攒 + 长期不变 |
| 5 真源文档拆解 | 🗣️ 挖掘 | 实施真源文档 | 先拆首个阶段，后续边做边拆 |
| 6 前端骨架 | 🗣️ 挖掘 → ⚡ 执行 | 前端架构实施文档 | 先文档后代码 + 边做边补 |
| 7 数据库 | 🗣️ 挖掘 → ⚡ 执行 | schema.sql + 数据库设计文档 | 先文档后代码 |
| 8 后端骨架 | 🗣️ 挖掘 + ⚡ 执行 | 后端架构实施文档 | 先文档后代码 + 边做边补 |
| 9 后端联调 | ⚡ 执行 | — | 联调验证 |
| 10 架构验收 | 🗣️ 挖掘 | 后端架构实施真源文档 + 前端架构实施真源文档 | 反推生成 + 持续同步 |
| 11 业务开发 | 🗣️ 挖掘 → ⚡ 执行（每轮） | 实施计划（可暂存） | 先计划后执行 + 边做边同步 |
| 12 安全审查 | 🗣️ 挖掘 | 后端安全审查报告 | 一次性生成 + 整改后追加 |
| 13 上线验收 | 🗣️ 挖掘 | 上线前验收报告 | 一次性生成 + 整改后追加 |
| 14 上线反哺 | 🗣️ 挖掘 | 复盘报告 + Skill 库 | 报告一次性 + Skill 持续迭代 |

### 17.2 "直接做" vs "对话挖掘" 全局清单

#### ⚡ 可以直接让 AI 做（规则已清晰）

- 创建项目文件夹 / 初始化项目
- 按文档搭骨架（文档已确认）
- 执行 schema.sql 建表
- 重复性工作（建文件、改命名、批量替换）
- 单一明确的 bug 修复（根因已定位）
- 格式化文档 / 整理清单 / 生成测试数据
- 按整改清单修问题（清单已确认）
- 生成验收报告 / 审查报告（规则已定义）

#### 🗣️ 必须对话挖掘（模糊或高风险）

- 项目方向 / 用户场景 / 功能边界
- 技术选型 / 框架复用边界
- Agent 宪法规则
- 设计 token / 目录规则 / 接口规则
- 业务对象 / 对象关系 / 状态字段
- 权限模型 / 数据归属 / 安全边界
- 多个方案权衡 / ROI 分析
- 任何"自己也说不清"的需求
- 异常处理策略 / 错误码设计
- 模块拆分 / 优先级排序

### 17.3 全局开场模板（每次对话必用）

```
当前阶段：[立项 / 选型 / 宪法 / 前端 / 数据库 / 后端 / 业务 / 验收 / 调试 / 反哺]
你的角色：[产品合伙人 / 架构顾问 / 数据库架构师 / 后端架构师 / 前端架构顾问 / 安全审查员 / 代码审查员 / 调试员 / 复盘员]
上下文：
- 立项文档：[贴或引用]
- 技术选型文档：[贴或引用]
- 真源文档：[贴或引用]
- Agent 宪法：[贴或引用]
约束：给唯一推荐，不要列多方案
本轮范围：只做 [具体任务]，不一次性做完
验收标准：[具体可检查标准]
```

### 17.4 全局收尾模板（每次宣布完成前必用）

```
在说"已完成"之前，请按以下清单自检：
1. 走验收清单，每项给证据（文件 / 命令 / 输出 / 日志）
2. 未验证项标记"未验证"，不允许写"基本完成"
3. 回归检查（相邻功能有没有被破坏）
4. 同步真源文档（如有规则变更）
5. Git 提交（commit message 用 feat/fix/chore 前缀）
6. 反哺 Skill（如发现盲点）

最后给明确结论：能否进入下一阶段 / 下一轮。
```

### 17.5 跨阶段执行原则（每个阶段都适用）

#### 原则 1：每推进一块，先验收再往下

不管是骨架、联调还是业务模块，每完成一个子阶段，都先让 AI 汇报**三件事**，再决定是否继续：

```
完成后汇报三件事：
1. 对照实施真源文档，这个阶段哪些做完了，哪些还没做完
2. 到底改了哪些地方，有没有漏做，有没有多做
3. 怎么验证的，验证结果是什么，下一个该做哪个子阶段，为什么
```

> 这三件事问完，你就能看出来它是在按文档往前走，还是已经开始凭感觉乱跑。如果它说的下一步和文档对不上，说明理解已经偏了，先拉回来再继续。

#### 原则 2：地基变更必须先评估再动手

业务细节可以随手调整，但**地基不能随手翻**。地基层包括：技术栈、目录结构、数据库表设计、核心字段、登录权限、支付流程。

如果要改的东西可能影响地基，**先停下来问 AI**：

```
这个改动会不会影响以下地基？
- 技术栈 / 框架版本
- 目录结构
- 数据库表设计 / 核心字段
- 登录 / 权限模型
- 支付 / 核心业务流程

如果会，先不要实现，先给影响评估和调整方案。
如果不会，说明为什么不会，然后继续。
```

> 判断不了就直接问 AI 讨论。宁可多问一句，也不要图省事让 AI 把架构破坏了。

#### 原则 3：防漂移指令（每个子阶段开工前必说）

```
严格遵守 Agent 宪法和实施真源文档，只实施当前子阶段，不要超出范围，做完后停下来汇报，不要自动进入下一个子阶段。
```

> 同时把当前阶段对应的实施真源文档路径一起发给 AI。两边都有据可依——它怎么做事（宪法）、这次具体做什么（真源文档），就不容易乱跑。

### 17.6 一句话心法（贴在显示器上）

> **AI 没记忆 → 文档是它的记忆**
> **AI 默认动手 → 你要逼它先想**
> **AI 默认讨好 → 你要逼它给判断**
> **AI 默认结论 → 你要逼它给证据**
> **AI 默认前进 → 你要逼它会停**
> **AI 会偷懒 → 一句话甩过去它就脑补，必须给够上下文和约束**
> **判断模糊先聊，规则清晰才做**
> **改代码前先改文档，改完代码再核文档**

---

## 使用建议

1. **每个新项目开始前**：先翻 [项目可视化流程手册.md](./项目可视化流程手册.md) 第 1 章判断项目状态
2. **每个阶段开始前**：翻本手册对应章节，确认挖掘点 / 执行清单 / 验收标准 / 文档策略
3. **每次对话开始前**：用 17.3 节开场模板
4. **每次对话收尾前**：用 17.4 节收尾模板
5. **遇到异常**：翻第 15 章找对应话术
6. **不确定文档怎么生成**：翻第 16 章判断策略
7. **快速查阶段**：用第 17 章速查表

---

> 配套文档：
> - [使用教程与导航手册.md](./使用教程与导航手册.md) — 新手入门 + 日常场景速查
> - [项目可视化流程手册.md](./项目可视化流程手册.md) — 走哪条路
> - [全流程AI对话Prompt模板库.md](./全流程AI对话Prompt模板库.md) — 可直接复制的 prompt 工具箱（含 17 条高频对话技巧）



